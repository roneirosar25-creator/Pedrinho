//+------------------------------------------------------------------+
//|                                      RING_BTCUSD_PRO.mq5         |
//|                          BTCUSD Optimizado (Cripto 24/7)         |
//+------------------------------------------------------------------+
#property copyright "RoneiRosar23"
#property version   "2.00"
#property description "Sistema Antecipatório Multi-Timeframe para BTCUSD"
...
#property indicator_label1  "BTC Buy Early"

#property indicator_type2   DRAW_ARROW
#property indicator_width2  2
#property indicator_color2  0xFF4444
#property indicator_label2  "BTC Sell Early"

#property indicator_type3   DRAW_ARROW
#property indicator_width3  1
#property indicator_color3  0x4488FF
#property indicator_label3  "BTC Buy Confirm"

#property indicator_type4   DRAW_ARROW
#property indicator_width4  1
#property indicator_color4  0xFF8800
#property indicator_label4  "BTC Sell Confirm"
...
input int      WPR_Period       = 8;
input int      RSI_Period       = 14;
input int      MACD_Fast        = 8;
input int      MACD_Slow        = 18;
input int      MACD_Signal      = 5;
input int      Overbought       = -10;
input int      Oversold         = -90;
input int      NeutralHigh      = -20;
input int      NeutralLow       = -80;
...
input int      MinScoreForEarly  = 50;
input int      MinScoreForEntry  = 70;
...
string labelBuy="BTC_TotalBuy",labelSell="BTC_TotalSell",obj_name="BTC_InfoBox";
...
int OnInit()
{
   ...
   if(wpr_handle==INVALID_HANDLE||rsi_handle==INVALID_HANDLE||macd_handle==INVALID_HANDLE||atr_handle==INVALID_HANDLE||ad_handle==INVALID_HANDLE)
      return INIT_FAILED;
   CreateMainLabel("BTC_TrendLabel","BTCUSD SISTEMA : INICIALIZANDO...",500,15,15,clrWhite);
   CreateMainLabel("BTC_PLLabel","BTCUSD P/L : ---",500,80,16,clrWhite);
   CreateMainLabel(labelBuy,"BTCUSD LOT BUY : 0.00",500,50,13,clrMistyRose);
   CreateMainLabel(labelSell,"BTCUSD LOT SELL : 0.00",500,65,13,clrMistyRose);
...
{
   ObjectsDeleteAll(0,"BTC_"); ObjectsDeleteAll(0,prefix);
...
int CalculateBuyScore(int i)
{
   ...
   if(RSIbuff[i]<20) sc+=15;
   else if(RSIbuff[i]<30) sc+=8;
...
int CalculateSellScore(int i)
{
   ...
   if(RSIbuff[i]>80) sc+=15;
   else if(RSIbuff[i]>70) sc+=8;
   if(i+1<ArraySize(ADbuff)&&ADbuff[i]<ADbuff[i+1]) sc+=10;
   if(i+2<ArraySize(WPRbuff))
   {
      double v=WPRbuff[i]-WPRbuff[i+1];
...
void UpdateGraphicObjects(const double&c[],const double&h[],const double&l[])
{
   ...
   if(bs>=MinScoreForEntry){tt="BTC SINAL FORTE COMPRA ["+IntegerToString(bs)+"/100]"; tc=clrDeepSkyBlue; st="BUY";}
   else if(ss>=MinScoreForEntry){tt="BTC SINAL FORTE VENDA ["+IntegerToString(ss)+"/100]"; tc=clrMagenta; st="SELL";}
   else if(bs>=MinScoreForEarly){tt="BTC ANTECIPACAO COMPRA ["+IntegerToString(bs)+"/100]"; tc=clrAqua; st="BUY_EARLY";}
   else if(ss>=MinScoreForEarly){tt="BTC ANTECIPACAO VENDA ["+IntegerToString(ss)+"/100]"; tc=clrOrange; st="SELL_EARLY";}
   else {tt="BTC NEUTRO [B:"+IntegerToString(bs)+" S:"+IntegerToString(ss)+"]"; tc=clrYellow;}
   if(StringFind(st,"BUY")>=0)
   {
      tp="TP:"+DoubleToString(c[0]+atr*ATR_TP_Ratio,1)+" ("+DoubleToString(ATR_TP_Ratio,1)+"ATR)";
      sl="SL:"+DoubleToString(c[0]-atr*ATR_SL_Ratio,1)+" ("+DoubleToString(ATR_SL_Ratio,1)+"ATR)";
   }
   ...
   string info="WPR:"+DoubleToString(wpr,2)+"|RSI:"+DoubleToString(rsi,2)+"|ATR:"+DoubleToString(atr,1)+"|"+tt+"|"+tp+"|"+sl;
   ObjectSetString(0,obj_name,OBJPROP_TEXT,info); ObjectSetInteger(0,obj_name,OBJPROP_COLOR,tc);
   ObjectSetString(0,"BTC_TrendLabel",OBJPROP_TEXT,tt); ObjectSetInteger(0,"BTC_TrendLabel",OBJPROP_COLOR,tc);
   double pft=0;
   ...
   ObjectSetString(0,"BTC_PLLabel",OBJPROP_TEXT,pl); ObjectSetInteger(0,"BTC_PLLabel",OBJPROP_COLOR,plc);
   ...
   ObjectDelete(0,"BTC_TP_Line"); ObjectDelete(0,"BTC_SL_Line");
   if(StringFind(st,"BUY")>=0)
   {
      ObjectCreate(0,"BTC_TP_Line",OBJ_HLINE,0,0,c[0]+atr*ATR_TP_Ratio);
      ObjectSetInteger(0,"BTC_TP_Line",OBJPROP_COLOR,clrLime); ObjectSetInteger(0,"BTC_TP_Line",OBJPROP_WIDTH,1); ObjectSetInteger(0,"BTC_TP_Line",OBJPROP_STYLE,STYLE_DASH);
      ObjectCreate(0,"BTC_SL_Line",OBJ_HLINE,0,0,c[0]-atr*ATR_SL_Ratio);
      ObjectSetInteger(0,"BTC_SL_Line",OBJPROP_COLOR,clrRed); ObjectSetInteger(0,"BTC_SL_Line",OBJPROP_WIDTH,1); ObjectSetInteger(0,"BTC_SL_Line",OBJPROP_STYLE,STYLE_DASH);
   }
   else if(StringFind(st,"SELL")>=0)
   {
      ObjectCreate(0,"BTC_TP_Line",OBJ_HLINE,0,0,c[0]-atr*ATR_TP_Ratio);
...
void DrawMTFDashboard()
{
   ...
   CreateLabel(prefix+"header"," BTCUSD MTF MOMENTUM MATRIX [Score]",x,y,clrWhite,clrDarkSlateGray,10,315); y+=rh;
...
int GetMTFScore(ENUM_TIMEFRAMES tf)
{
   ...
   if(rsi[0]<20) sc+=15; else if(rsi[0]<30) sc+=5;
   if(ad[0]>ad[1]) sc+=10;
   if(wpr[0]>wpr[1]&&wpr[0]<-50) sc+=10;
   if(wpr[0]>Overbought) sc-=25; else if(wpr[0]>-50) sc-=10;
   if(rsi[0]>80) sc-=15; else if(rsi[0]>70) sc-=5;
   if(ad[0]<ad[1]) sc-=10;
   if(wpr[0]<wpr[1]&&wpr[0]>-50) sc-=10;
