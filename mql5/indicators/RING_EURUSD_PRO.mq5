//+------------------------------------------------------------------+
//|                                      RING_EURUSD_PRO.mq5         |
//|                          EURUSD Optimizado (Forex 5-dígitos)     |
//+------------------------------------------------------------------+
#property copyright "RoneiRosar23"
#property version   "2.00"
#property description "Sistema Antecipatório Multi-Timeframe para EURUSD"
...
#property indicator_label1  "EURUSD Buy Early"

#property indicator_type2   DRAW_ARROW
#property indicator_width2  2
#property indicator_color2  0xFF4444
#property indicator_label2  "EURUSD Sell Early"

#property indicator_type3   DRAW_ARROW
#property indicator_width3  1
#property indicator_color3  0x4488FF
#property indicator_label3  "EURUSD Buy Confirm"

#property indicator_type4   DRAW_ARROW
#property indicator_width4  1
#property indicator_color4  0xFF8800
#property indicator_label4  "EURUSD Sell Confirm"
...
input int      Overbought       = -20;
input int      Oversold         = -80;
input int      NeutralHigh      = -30;
input int      NeutralLow       = -70;
input double   ATR_TP_Ratio     = 2.0;
input double   ATR_SL_Ratio     = 1.0;
input int      RambooShift      = 14;
input int      DashX            = 500;
input int      DashY            = 200;
input int      MainBoxX         = 380;
input int      MainBoxY         = 113;
input int      MinScoreForEarly  = 60;
input int      MinScoreForEntry  = 80;

string prefix="EURPRO_";
...
string labelBuy="EUR_TotalBuy",labelSell="EUR_TotalSell",obj_name="EUR_InfoBox";
...
int OnInit()
{
   ...
   if(wpr_handle==INVALID_HANDLE||rsi_handle==INVALID_HANDLE||macd_handle==INVALID_HANDLE||atr_handle==INVALID_HANDLE||ad_handle==INVALID_HANDLE)
      return INIT_FAILED;
   CreateMainLabel("EUR_TrendLabel","EURUSD SISTEMA : INICIALIZANDO...",500,15,15,clrWhite);
   CreateMainLabel("EUR_PLLabel","EURUSD P/L : ---",500,80,16,clrWhite);
   CreateMainLabel(labelBuy,"EURUSD LOT BUY : 0.00",500,50,13,clrMistyRose);
   CreateMainLabel(labelSell,"EURUSD LOT SELL : 0.00",500,65,13,clrMistyRose);
...
{
   ObjectsDeleteAll(0,"EUR_"); ObjectsDeleteAll(0,prefix);
...
int CalculateBuyScore(int i)
{
   ...
   if(RSIbuff[i]<30) sc+=15;
   else if(RSIbuff[i]<40) sc+=8;
...
int CalculateSellScore(int i)
{
   ...
   if(RSIbuff[i]>70) sc+=15;
   else if(RSIbuff[i]>60) sc+=8;
   if(i+1<ArraySize(ADbuff)&&ADbuff[i]<ADbuff[i+1]) sc+=10;
   if(i+2<ArraySize(WPRbuff))
   {
      double v=WPRbuff[i]-WPRbuff[i+1];
...
void UpdateGraphicObjects(const double&c[],const double&h[],const double&l[])
{
   ...
   if(bs>=MinScoreForEntry){tt="EURUSD SINAL FORTE COMPRA ["+IntegerToString(bs)+"/100]"; tc=clrDeepSkyBlue; st="BUY";}
   else if(ss>=MinScoreForEntry){tt="EURUSD SINAL FORTE VENDA ["+IntegerToString(ss)+"/100]"; tc=clrMagenta; st="SELL";}
   else if(bs>=MinScoreForEarly){tt="EURUSD ANTECIPACAO COMPRA ["+IntegerToString(bs)+"/100]"; tc=clrAqua; st="BUY_EARLY";}
   else if(ss>=MinScoreForEarly){tt="EURUSD ANTECIPACAO VENDA ["+IntegerToString(ss)+"/100]"; tc=clrOrange; st="SELL_EARLY";}
   else {tt="EURUSD NEUTRO [B:"+IntegerToString(bs)+" S:"+IntegerToString(ss)+"]"; tc=clrYellow;}
   if(StringFind(st,"BUY")>=0)
   {
      tp="TP:"+DoubleToString(c[0]+atr*ATR_TP_Ratio,5)+" ("+DoubleToString(ATR_TP_Ratio,1)+"ATR)";
      sl="SL:"+DoubleToString(c[0]-atr*ATR_SL_Ratio,5)+" ("+DoubleToString(ATR_SL_Ratio,1)+"ATR)";
   }
   ...
   string info="WPR:"+DoubleToString(wpr,2)+"|RSI:"+DoubleToString(rsi,2)+"|ATR:"+DoubleToString(atr,5)+"|"+tt+"|"+tp+"|"+sl;
   ObjectSetString(0,obj_name,OBJPROP_TEXT,info); ObjectSetInteger(0,obj_name,OBJPROP_COLOR,tc);
   ObjectSetString(0,"EUR_TrendLabel",OBJPROP_TEXT,tt); ObjectSetInteger(0,"EUR_TrendLabel",OBJPROP_COLOR,tc);
   double pft=0;
   ...
   ObjectSetString(0,"EUR_PLLabel",OBJPROP_TEXT,pl); ObjectSetInteger(0,"EUR_PLLabel",OBJPROP_COLOR,plc);
   ...
   ObjectDelete(0,"EUR_TP_Line"); ObjectDelete(0,"EUR_SL_Line");
   if(StringFind(st,"BUY")>=0)
   {
      ObjectCreate(0,"EUR_TP_Line",OBJ_HLINE,0,0,c[0]+atr*ATR_TP_Ratio);
      ObjectSetInteger(0,"EUR_TP_Line",OBJPROP_COLOR,clrLime); ObjectSetInteger(0,"EUR_TP_Line",OBJPROP_WIDTH,1); ObjectSetInteger(0,"EUR_TP_Line",OBJPROP_STYLE,STYLE_DASH);
      ObjectCreate(0,"EUR_SL_Line",OBJ_HLINE,0,0,c[0]-atr*ATR_SL_Ratio);
      ObjectSetInteger(0,"EUR_SL_Line",OBJPROP_COLOR,clrRed); ObjectSetInteger(0,"EUR_SL_Line",OBJPROP_WIDTH,1); ObjectSetInteger(0,"EUR_SL_Line",OBJPROP_STYLE,STYLE_DASH);
   }
   else if(StringFind(st,"SELL")>=0)
   {
      ObjectCreate(0,"EUR_TP_Line",OBJ_HLINE,0,0,c[0]-atr*ATR_TP_Ratio);
...
void DrawMTFDashboard()
{
   ...
   CreateLabel(prefix+"header"," EURUSD MTF MOMENTUM MATRIX [Score]",x,y,clrWhite,clrDarkSlateGray,10,315); y+=rh;
...
int GetMTFScore(ENUM_TIMEFRAMES tf)
{
   ...
   int ha=iAD(_Symbol,tf,VOLUME_TICK); int hAt=iATR(_Symbol,tf,RambooShift);
   if(hw==INVALID_HANDLE||hr==INVALID_HANDLE||ha==INVALID_HANDLE||hAt==INVALID_HANDLE)
   {IndicatorRelease(hw);IndicatorRelease(hr);IndicatorRelease(ha);IndicatorRelease(hAt);return 0;}
   if(CopyBuffer(hw,0,0,2,wpr)<=0||CopyBuffer(hr,0,0,1,rsi)<=0||CopyBuffer(ha,0,0,2,ad)<=0||CopyBuffer(hAt,0,0,1,atr)<=0)
   {IndicatorRelease(hw);IndicatorRelease(hr);IndicatorRelease(ha);IndicatorRelease(hAt);return 0;}
   ArraySetAsSeries(wpr,true); ArraySetAsSeries(ad,true);
   int sc=0;
   if(wpr[0]<Oversold) sc+=25; else if(wpr[0]<-50) sc+=10;
   if(rsi[0]<30) sc+=15; else if(rsi[0]<45) sc+=5;
   if(ad[0]>ad[1]) sc+=10;
   if(wpr[0]>wpr[1]&&wpr[0]<-50) sc+=10;
   if(wpr[0]>Overbought) sc-=25; else if(wpr[0]>-50) sc-=10;
   if(rsi[0]>70) sc-=15; else if(rsi[0]>55) sc-=5;
   if(ad[0]<ad[1]) sc-=10;
   if(wpr[0]<wpr[1]&&wpr[0]>-50) sc-=10;
   IndicatorRelease(hw); IndicatorRelease(hr); IndicatorRelease(ha); IndicatorRelease(hAt);
   return sc;
}

double GetMTFWPR(ENUM_TIMEFRAMES tf)
{
   int h=iWPR(_Symbol,tf,WPR_Period); double b[];
   if(CopyBuffer(h,0,0,1,b)<=0) {IndicatorRelease(h); return 0;}
   IndicatorRelease(h); return b[0];
}

double GetMTFRSI(ENUM_TIMEFRAMES tf)
{
   int h=iRSI(_Symbol,tf,RSI_Period,PRICE_CLOSE); double b[];
   if(CopyBuffer(h,0,0,1,b)<=0) {IndicatorRelease(h); return 0;}
   IndicatorRelease(h); return b[0];
}

void CreateMainLabel(string n,string t,int x,int y,int fs,color c)
{
   ObjectDelete(0,n); ObjectCreate(0,n,OBJ_LABEL,0,0,0);
   ObjectSetInteger(0,n,OBJPROP_CORNER,CORNER_LEFT_UPPER); ObjectSetInteger(0,n,OBJPROP_XDISTANCE,x);
...
void CreateLabel(string n,string t,int x,int y,color tc,color bc,int sz,int wd)
{
   ObjectDelete(0,n); ObjectCreate(0,n,OBJ_EDIT,0,0,0);
   ObjectSetInteger(0,n,OBJPROP_XDISTANCE,x); ObjectSetInteger(0,n,OBJPROP_YDISTANCE,y);
   ObjectSetInteger(0,n,OBJPROP_XSIZE,wd); ObjectSetInteger(0,n,OBJPROP_YSIZE,18);
   ObjectSetInteger(0,n,OBJPROP_BGCOLOR,bc); ObjectSetInteger(0,n,OBJPROP_BORDER_TYPE,BORDER_FLAT);
