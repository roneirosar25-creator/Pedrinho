//+------------------------------------------------------------------+
//| NEXUS369_REVERSAO_REAL.mq5                                        |
//| TRIVIUM369 (c) 2026 - VERSAO EXCLUSIVA PRA CONTA REAL              |
//|                                                                    |
//| NUNCA renomear de volta pra NEXUS369_REVERSAO_EA nem instalar nas  |
//| contas demo - nome, kill switch, Magic Number e arquivos de log    |
//| SAO TODOS DIFERENTES de proposito (pedido do Ronei 14/07/2026,     |
//| "missao delicada", conta real com saldo muito pequeno).            |
//|                                                                    |
//| SEGURANCA EXTRA (so nesta versao): CalcularLote() RECUSA a entrada |
//| (retorna 0) se o risco% pedido nao justificar nem o lote minimo    |
//| da corretora, em vez de forcar o minimo (que arriscaria mais que   |
//| o combinado num saldo pequeno).                                   |
//|                                                                    |
...
#define GV_EXECUCAO_AUTORIZADA_REVERSAO "TRIVIUM369_EXECUCAO_AUTORIZADA_REVERSAO_REAL"
...
// 15/07/2026 - CORRIGIDO (mesmo bug de magic duplicado do TREND_REAL,
...
int OnInit()
{
   ...
   // 15/07/2026 - CORRIGIDO (bug 4802 confirmado via TRIVIUM_TESTE_ICUSTOM):
   // "Indicators\\" duplicava o caminho, ja assumido por padrao no iCustom.
   ...
   logFileName = "REAL_NEXUS369_REVERSAO_RELATORIO_" + sym + "_" + IntegerToString(contaLogin) + ".txt";
...
// 16/07/2026 - CORRIGIDO (mesma auditoria da versao demo).
...
double CalcularLote(double riskPct, double slDistPoints)
{
   ...
   // 14/07/2026 - Ronei pediu pra desligar a trava (ver mesma decisao no
   // NEXUS369_TREND_REAL.mq5) - forca o lote minimo mesmo que isso passe
   // do InpRiskPercent combinado, dado o saldo pequeno.
   if(lots < minLot)
...
void OnTick()
{
   ...
   if(TendenciaTemPosicaoAberta())
   {
      ...
      if(curBarChk != lastSignalBarTime)
      {
