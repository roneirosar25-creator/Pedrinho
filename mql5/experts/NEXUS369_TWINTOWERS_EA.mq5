//+------------------------------------------------------------------+
//| NEXUS369_TWINTOWERS_EA.mq5                                        |
//| TRIVIUM369 (c) 2026 - Pedrinho, 20/07/2026 (madrugada, autonomo)   |
//|                                                                    |
//| Setup "Twin Towers" do curso Setups Matadores (L&S): o mercado    |
//| gera uma vela de forca pra um lado e a vela seguinte nega esse     |
//| sinal com uma barra igual ou mais forte na direcao oposta - como   |
//| se o mercado "driblasse". Direcao = direcao da SEGUNDA vela (a de  |
//| reversao, que "venceu o dribble").                                 |
//|                                                                    |
//| REGRA CODIFICADA (traduzi a descricao qualitativa do curso em      |
//| numeros - registrado, nao e valor do curso): vela anterior (j-1)   |
//| com corpo>=1.0xATR14; vela de sinal (j) de cor OPOSTA com corpo >= |
//| corpo da vela anterior. Entra na abertura da vela seguinte (j+1)   |
//| na direcao da vela de sinal.                                       |
//|                                                                    |
//| RESULTADO DO BACKTEST (20/07/2026, H1, spread real, treino/teste   |
//| 70/30): aprovado em 12 dos 14 pares testados (NZDUSD e GBPNZD      |
//| reprovaram - fora da tabela). PF treino 1.26-2.34, PF teste        |
//| 1.10-2.51, WR 41-57%. Amostra menor que os outros setups desta     |
//| familia (~300-770 trades por par - o padrao e mais raro), mas      |
//| consistente nos dois lados.                                        |
//|                                                                    |
//| SAIDA EM 2 FASES (mesmo framework validado no Precursor/           |
//| Estilingue/RompimentoFalso).                                       |
//|                                                                    |
//| KILL SWITCH PROPRIO (GV_EXECUCAO_AUTORIZADA_TWINTOWERS).           |
//+------------------------------------------------------------------+
...
input group "=== Filtro do sinal (usado so se auto-calibragem = false) ==="
input int    InpATRPeriod        = 14;
input double InpForcaMinPrimeira = 1.0; // corpo da 1a vela >= X * ATR14
...
double g_ForcaMinPrimeira;
double g_RiskPercent;
bool   g_ParOperavel = true;

//+------------------------------------------------------------------+
// Tabela de calibragem - 12 pares aprovados no backtest (20/07/2026,
// H1). NZDUSD e GBPNZD reprovaram (PF<1 no teste) - ficam de fora.
//+------------------------------------------------------------------+
void CarregarCalibragemPorAtivo()
{
   if(!InpAutoCalibrarPorAtivo) { g_ForcaMinPrimeira = InpForcaMinPrimeira; g_RiskPercent = InpRiskPercent; return; }

   string sym = _Symbol; StringToUpper(sym);
   g_ParOperavel = true;

   if(StringFind(sym, "EURUSD") >= 0)      { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "GBPUSD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "USDCAD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "AUDUSD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "EURAUD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "EURCAD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "EURCHF") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "EURGBP") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "EURNZD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "GBPAUD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "GBPCAD") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else if(StringFind(sym, "USDCHF") >= 0) { g_ForcaMinPrimeira = 1.0; g_RiskPercent = 1.0; }
   else
   {
      g_ForcaMinPrimeira = 999.0; g_RiskPercent = 0.0; g_ParOperavel = false;
   }
}

input group "=== Magic Number ==="
input string InpSetupTipo     = "TwinTowers";
...
#define GV_EXECUCAO_AUTORIZADA_TWINTOWERS "TRIVIUM369_EXECUCAO_AUTORIZADA_TWINTOWERS"
...
bool ExecucaoAutorizada()
{
   ...
   return GlobalVariableCheck(GV_EXECUCAO_AUTORIZADA_TWINTOWERS) && GlobalVariableGet(GV_EXECUCAO_AUTORIZADA_TWINTOWERS) >= 1.0;
}
...
{
   string nome = StringFormat("TRIVIUM_TT_ENTRADA_%d", magic);
   ...
   string rotulo = StringFormat("TRIVIUM_TT_ENTRADA_TXT_%d", magic);
   ObjectCreate(0, rotulo, OBJ_TEXT, 0, tempo, preco);
   ObjectSetString(0, rotulo, OBJPROP_TEXT, StringFormat(" TWIN TOWERS %s #%d", (lado == POSITION_TYPE_BUY) ? "COMPRA" : "VENDA", magic));
...
#define PAINEL_TT_NOME "TRIVIUM_PAINEL_TWINTOWERS"
void AtualizaParecer(string texto)
{
   if(ObjectFind(0, PAINEL_TT_NOME) < 0)
   {
      ObjectCreate(0, PAINEL_TT_NOME, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_XDISTANCE, 5);
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_YDISTANCE, 210);
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_FONTSIZE, 8);
      ObjectSetString(0, PAINEL_TT_NOME, OBJPROP_FONT, "Consolas");
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_HIDDEN, true);
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_COLOR, clrMagenta);
      ObjectSetInteger(0, PAINEL_TT_NOME, OBJPROP_BACK, false);
   }
   ObjectSetString(0, PAINEL_TT_NOME, OBJPROP_TEXT, "[TWIN TOWERS]\n" + texto);
}
...
int OnInit()
{
   ...
   if(!g_ParOperavel)
      Print("AVISO: ", sym, " fora da tabela validada do NEXUS369_TWINTOWERS_EA (12 pares forex H1: EURUSD/GBPUSD/USDCAD/AUDUSD/EURAUD/EURCAD/EURCHF/EURGBP/EURNZD/GBPAUD/GBPCAD/USDCHF-T; NZDUSD e GBPNZD reprovaram) - grafico so visual, EA nao abre posicao aqui.");
   ...
   logFileName = "NEXUS369_TWINTOWERS_RELATORIO_" + sym + "_" + IntegerToString(contaLogin) + ".txt";
   g_signalsLogFile = "TRIVIUM369_SIGNALS_LOG_" + IntegerToString(contaLogin) + ".csv";
   g_gvLosses = GV_LOSSES_PREFIX + IntegerToString(contaLogin) + "_" + sym + "_TT";

   LogRelatorio(StringFormat("=== INIT TWINTOWERS %s | %s %s | ForcaMin=%.1fxATR | Fase1=%dvelas@%.1fxATR Fase2=trailing%.1fxATR | Risco=%.1f%% | EXECUCAO=%s ===",
      TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES), sym, NomePeriodo(PERIOD_OPERACIONAL),
      g_ForcaMinPrimeira, InpBarrasGraca, InpStopGracaATRMult, InpTrailingATRMult, g_RiskPercent,
...
void OnTick()
{
   ...
   double atr_buf[1];
   if(CopyBuffer(h_atr, 0, 2, 1, atr_buf) < 1) return; // ATR calculado na 1a vela (j-1)
   double atr_ref = atr_buf[0];
   if(atr_ref <= 0) return;

   MqlRates velas[2]; // [0]=vela de sinal (indice 1), [1]=vela anterior (indice 2)
   if(CopyRates(_Symbol, PERIOD_OPERACIONAL, 1, 2, velas) < 2) return;
   MqlRates velaSinal = velas[1], velaAnterior = velas[0]; // CopyRates sem series: [0]=mais antiga

   double corpoAnterior = MathAbs(velaAnterior.close - velaAnterior.open);
   double corpoSinal = MathAbs(velaSinal.close - velaSinal.open);
   bool corpoAltaAnterior = velaAnterior.close > velaAnterior.open;
   bool corpoAltaSinal = velaSinal.close > velaSinal.open;

   bool primeiraForte = corpoAnterior >= g_ForcaMinPrimeira * atr_ref;
   bool segundaNega = (corpoAltaSinal != corpoAltaAnterior) && (corpoSinal >= corpoAnterior);
   if(!(primeiraForte && segundaNega)) return;

   string side = corpoAltaSinal ? "BUY" : "SELL";
   ...
   string comentario = StringFormat("TT %s %s", tf, side);
   if(StringLen(comentario) > 31) comentario = StringSubstr(comentario, 0, 31);

   string motivoDetalhado = StringFormat(
      "Twin Towers: vela anterior forte (corpo=%.2fx ATR14, min %.1fx) na direcao %s, vela de sinal negou com corpo igual ou maior (%.2fx ATR14) na direcao oposta - entra a favor da 2a vela: %s",
      corpoAnterior/atr_ref, g_ForcaMinPrimeira, corpoAltaAnterior?"ALTA":"BAIXA", corpoSinal/atr_ref, side);
...
double OnTester()
{
   ...
   if(h != INVALID_HANDLE)
   {
      ...
      FileWriteString(h, StringFormat("%s;%s;%s;%.1f;%d;%.2f;%.2f;%.2f;%.2f;%.2f;%d;%.2f;%.2f;%d\r\n",
         _Symbol, EnumToString(PERIOD_OPERACIONAL), "TWINTOWERS", g_RiskPercent, (int)AccountInfoInteger(ACCOUNT_LEVERAGE), deposito,
