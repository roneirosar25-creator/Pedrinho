//+------------------------------------------------------------------+
//| NEXUS369_ROMPIMENTOFALSO_EA.mq5                                   |
//| TRIVIUM369 (c) 2026 - Pedrinho, 20/07/2026 (madrugada, autonomo)   |
//|                                                                    |
//| Resultado mais forte e mais universal encontrado nos cursos L&S/   |
//| LibertaPlay: rompimento falso GENERICO (sem exigir squeeze de      |
//| Bollinger como o NEXUS369_ESTILINGUE_EA) - vela rompe a maxima ou  |
//| minima dos ultimos 20 candles mas fecha de volta pra dentro do     |
//| range (frustracao do rompimento). Direcao = contraria ao           |
//| rompimento que falhou.                                             |
//|                                                                    |
//| RESULTADO DO BACKTEST (20/07/2026, H1, spread real, treino/teste   |
//| 70/30): APROVADO NOS 14 PARES TESTADOS (unico setup desta sessao   |
//| que passou em 100% do universo testado), ~130k+ trades no total.   |
//| PF treino 1.21-1.79, PF teste 1.04-1.71, WR 40-51%. Pares com JPY  |
//| nao foram testados aqui (ja reprovaram sistematicamente no         |
//| Precursor e no Estilingue - mesma familia de regra, nao vale a     |
//| pena testar de novo).                                              |
//|                                                                    |
//| SAIDA EM 2 FASES (mesmo framework validado no Precursor/           |
//| Estilingue): fase 1 (3 primeiras velas) stop largo fixo 3.5xATR;   |
//| fase 2 (4a vela em diante) trailing 0.5xATR do pico a favor.       |
//|                                                                    |
//| KILL SWITCH PROPRIO (GV_EXECUCAO_AUTORIZADA_ROMPIMENTOFALSO),      |
//| separado de todos os outros EAs - precisa de "pode rodar" novo.    |
//+------------------------------------------------------------------+
...
input group "=== Rompimento (usado so se auto-calibragem = false) ==="
input int    InpJanelaRompimento  = 20; // maxima/minima dos ultimos N candles
...
// (20/07/2026, H1). Unico setup da sessao aprovado em 100% do
// universo testado - JPY nao foi testado aqui pois ja reprovou
// sistematicamente nas regras irmas (Precursor/Estilingue).
//+------------------------------------------------------------------+
...
input string InpSetupTipo     = "RompimentoFalso_Generico";
...
#define GV_EXECUCAO_AUTORIZADA_ROMPIMENTOFALSO "TRIVIUM369_EXECUCAO_AUTORIZADA_ROMPIMENTOFALSO"
...
bool ExecucaoAutorizada()
{
   ...
   return GlobalVariableCheck(GV_EXECUCAO_AUTORIZADA_ROMPIMENTOFALSO) && GlobalVariableGet(GV_EXECUCAO_AUTORIZADA_ROMPIMENTOFALSO) >= 1.0;
}
...
{
   string nome = StringFormat("TRIVIUM_RF_ENTRADA_%d", magic);
   ...
   string rotulo = StringFormat("TRIVIUM_RF_ENTRADA_TXT_%d", magic);
   ObjectCreate(0, rotulo, OBJ_TEXT, 0, tempo, preco);
   ObjectSetString(0, rotulo, OBJPROP_TEXT, StringFormat(" ROMP.FALSO %s #%d", (lado == POSITION_TYPE_BUY) ? "COMPRA" : "VENDA", magic));
...
#define PAINEL_RF_NOME "TRIVIUM_PAINEL_ROMPIMENTOFALSO"
void AtualizaParecer(string texto)
{
   if(ObjectFind(0, PAINEL_RF_NOME) < 0)
   {
      ObjectCreate(0, PAINEL_RF_NOME, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_XDISTANCE, 5);
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_YDISTANCE, 170);
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_FONTSIZE, 8);
      ObjectSetString(0, PAINEL_RF_NOME, OBJPROP_FONT, "Consolas");
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_HIDDEN, true);
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_COLOR, clrOrange);
      ObjectSetInteger(0, PAINEL_RF_NOME, OBJPROP_BACK, false);
   }
   ObjectSetString(0, PAINEL_RF_NOME, OBJPROP_TEXT, "[ROMPIMENTO FALSO]\n" + texto);
}
...
int OnInit()
{
   ...
   if(!g_ParOperavel)
      Print("AVISO: ", sym, " fora da tabela validada do NEXUS369_ROMPIMENTOFALSO_EA (14 pares forex H1: EURUSD/GBPUSD/USDCAD/AUDUSD/NZDUSD/EURAUD/EURCAD/EURCHF/EURGBP/EURNZD/GBPAUD/GBPCAD/GBPNZD/USDCHF-T) - grafico so visual, EA nao abre posicao aqui.");
   ...
   logFileName = "NEXUS369_ROMPIMENTOFALSO_RELATORIO_" + sym + "_" + IntegerToString(contaLogin) + ".txt";
   g_signalsLogFile = "TRIVIUM369_SIGNALS_LOG_" + IntegerToString(contaLogin) + ".csv";
   g_gvLosses = GV_LOSSES_PREFIX + IntegerToString(contaLogin) + "_" + sym + "_RF";

   LogRelatorio(StringFormat("=== INIT ROMPIMENTOFALSO %s | %s %s | Janela=%d velas | Fase1=%dvelas@%.1fxATR Fase2=trailing%.1fxATR | Risco=%.1f%% | EXECUCAO=%s ===",
      TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES), sym, NomePeriodo(PERIOD_OPERACIONAL),
      InpJanelaRompimento, InpBarrasGraca, InpStopGracaATRMult, InpTrailingATRMult, g_RiskPercent,
...
void OnTick()
{
   ...
   // Maxima/minima dos N candles ANTERIORES a vela de sinal (indice 2 ate InpJanelaRompimento+1)
   MqlRates janela[];
   ArraySetAsSeries(janela, true);
   if(CopyRates(_Symbol, PERIOD_OPERACIONAL, 2, InpJanelaRompimento, janela) < InpJanelaRompimento) return;
   double max20 = janela[0].high, min20 = janela[0].low;
   for(int i = 1; i < InpJanelaRompimento; i++) { max20 = MathMax(max20, janela[i].high); min20 = MathMin(min20, janela[i].low); }

   MqlRates velaSinal[1];
   if(CopyRates(_Symbol, PERIOD_OPERACIONAL, 1, 1, velaSinal) < 1) return;

   bool tentouCima  = (velaSinal[0].high > max20) && (velaSinal[0].close < max20);
   bool tentouBaixo = (velaSinal[0].low  < min20) && (velaSinal[0].close > min20);

   string side = "";
   if(tentouCima) side = "SELL";
   else if(tentouBaixo) side = "BUY";
   else return;
   ...
   string comentario = StringFormat("RF %s %s", tf, side);
   if(StringLen(comentario) > 31) comentario = StringSubstr(comentario, 0, 31);

   string motivoDetalhado = StringFormat(
      "Rompimento falso: vela anterior tentou romper %s dos ultimos %d candles (max=%s min=%s) e fechou de volta pra dentro - entra contraria: %s",
      tentouCima ? "MAXIMA" : "MINIMA", InpJanelaRompimento, DoubleToString(max20, _Digits), DoubleToString(min20, _Digits), side);
...
double OnTester()
{
   ...
   if(h != INVALID_HANDLE)
   {
      ...
      FileWriteString(h, StringFormat("%s;%s;%s;%.1f;%d;%.2f;%.2f;%.2f;%.2f;%.2f;%d;%.2f;%.2f;%d\r\n",
         _Symbol, EnumToString(PERIOD_OPERACIONAL), "ROMPIMENTOFALSO", g_RiskPercent, (int)AccountInfoInteger(ACCOUNT_LEVERAGE), deposito,
