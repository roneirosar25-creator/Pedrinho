//+------------------------------------------------------------------+
//| NEXUS369_ROMPIMENTOSR_EA.mq5                                      |
//| TRIVIUM369 (c) 2026 - Pedrinho, 21/07/2026                        |
//|                                                                    |
//| Setup "Rompimento" do curso Setups Matadores (L&S) - descartado    |
//| numa primeira analise por depender de S/R "subjetivo", mas o       |
//| projeto ja tem um algoritmo de deteccao de S/R VALIDADO            |
//| (TRIVIUM_DESENHAR_SR.mq5: fractal 5 velas de cada lado, funde      |
//| niveis dentro de 0.25%, janela H1=120 velas, max 3 niveis por      |
//| lado) - reaproveitado aqui pra tornar o setup testavel de verdade. |
//|                                                                    |
//| REGRA (curso: "boas estruturas rompidas costumam ter pelo menos    |
//| dois testes previos... o candle que produziu o rompimento deve     |
//| ser amplo e com bom volume"):                                      |
//|  1. Nivel de S/R fractal (mesmo algoritmo do TRIVIUM_DESENHAR_SR)  |
//|     com >=2 TESTES previos (toques dentro da tolerancia, depois    |
//|     do nivel confirmado, antes do rompimento).                     |
//|  2. Vela de rompimento: fecha alem do nivel, range>=1.2xATR14,     |
//|     volume>=1.2x media20 (numeros meus - traduzem "amplo e bom     |
//|     volume" do curso em regra testavel).                           |
//|  3. Direcao = direcao do rompimento. Entra na abertura da vela     |
//|     seguinte.                                                       |
//|                                                                    |
//| RESULTADO DO BACKTEST (21/07/2026, H1, spread real, treino/teste   |
//| 70/30): APROVADO NOS 14 DE 14 PARES TESTADOS (segundo setup da     |
//| sessao a passar em 100% do universo, depois do Rompimento Falso    |
//| generico). PF treino 1.29-1.9, PF teste 1.05-1.92, WR 40-51%,      |
//| ~2500-3500 trades por par.                                          |
//|                                                                    |
//| SAIDA EM 2 FASES (mesmo framework validado no resto da familia).   |
//|                                                                    |
//| KILL SWITCH PROPRIO (GV_EXECUCAO_AUTORIZADA_ROMPIMENTOSR).         |
//+------------------------------------------------------------------+
...
input group "=== Deteccao de S/R (mesmo algoritmo do TRIVIUM_DESENHAR_SR) ==="
input int    InpFractalAsas    = 5;     // barras de cada lado pra confirmar topo/fundo
input int    InpJanelaBarras   = 120;   // historico analisado pra achar niveis (mesmo default H1 do DESENHAR_SR)
input double InpTolerancaPct   = 0.25;  // % de proximidade pra considerar dois niveis "o mesmo"
input int    InpMaxNiveis      = 3;     // limite de niveis simultaneos por lado
input int    InpMinTestes      = 2;     // minimo de toques previos no nivel antes do rompimento valer

input group "=== Filtro de forca da vela de rompimento ==="
input int    InpATRPeriod      = 14;
input int    InpVolumePeriod   = 20;
input double InpForcaMin       = 1.2;   // range>=X*ATR14 E volume>=X*media20
...
//+------------------------------------------------------------------+
// Tabela de calibragem - os 14 pares abaixo passaram no backtest
// (21/07/2026, H1). Unico setup da familia (com o Rompimento Falso
// generico) aprovado em 100% do universo testado.
//+------------------------------------------------------------------+
...
input string InpSetupTipo     = "Rompimento_SR_Fractal";
...
#define GV_EXECUCAO_AUTORIZADA_ROMPIMENTOSR "TRIVIUM369_EXECUCAO_AUTORIZADA_ROMPIMENTOSR"
...
bool ExecucaoAutorizada()
{
   ...
   return GlobalVariableCheck(GV_EXECUCAO_AUTORIZADA_ROMPIMENTOSR) && GlobalVariableGet(GV_EXECUCAO_AUTORIZADA_ROMPIMENTOSR) >= 1.0;
}
...
{
   string nome = StringFormat("TRIVIUM_RSR_ENTRADA_%d", magic);
   ...
   string rotulo = StringFormat("TRIVIUM_RSR_ENTRADA_TXT_%d", magic);
   ObjectCreate(0, rotulo, OBJ_TEXT, 0, tempo, preco);
   ObjectSetString(0, rotulo, OBJPROP_TEXT, StringFormat(" ROMP.S/R %s #%d", (lado == POSITION_TYPE_BUY) ? "COMPRA" : "VENDA", magic));
...
#define PAINEL_RSR_NOME "TRIVIUM_PAINEL_ROMPIMENTOSR"
void AtualizaParecer(string texto)
{
   if(ObjectFind(0, PAINEL_RSR_NOME) < 0)
   {
      ObjectCreate(0, PAINEL_RSR_NOME, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_CORNER, CORNER_LEFT_UPPER);
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_XDISTANCE, 5);
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_YDISTANCE, 290);
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_FONTSIZE, 8);
      ObjectSetString(0, PAINEL_RSR_NOME, OBJPROP_FONT, "Consolas");
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_HIDDEN, true);
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_COLOR, clrDeepSkyBlue);
      ObjectSetInteger(0, PAINEL_RSR_NOME, OBJPROP_BACK, false);
   }
   ObjectSetString(0, PAINEL_RSR_NOME, OBJPROP_TEXT, "[ROMPIMENTO S/R]\n" + texto);
}
...
int OnInit()
{
   ...
   if(!g_ParOperavel)
      Print("AVISO: ", sym, " fora da tabela validada do NEXUS369_ROMPIMENTOSR_EA (14 pares forex H1: EURUSD/GBPUSD/USDCAD/AUDUSD/NZDUSD/EURAUD/EURCAD/EURCHF/EURGBP/EURNZD/GBPAUD/GBPCAD/GBPNZD/USDCHF-T) - grafico so visual, EA nao abre posicao aqui.");
   ...
   logFileName = "NEXUS369_ROMPIMENTOSR_RELATORIO_" + sym + "_" + IntegerToString(contaLogin) + ".txt";
   g_signalsLogFile = "TRIVIUM369_SIGNALS_LOG_" + IntegerToString(contaLogin) + ".csv";
   g_gvLosses = GV_LOSSES_PREFIX + IntegerToString(contaLogin) + "_" + sym + "_RSR";

   LogRelatorio(StringFormat("=== INIT ROMPIMENTOSR %s | %s %s | Fractal asas=%d janela=%d min_testes=%d | Fase1=%dvelas@%.1fxATR Fase2=trailing%.1fxATR | Risco=%.1f%% | EXECUCAO=%s ===",
      TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES), sym, NomePeriodo(PERIOD_OPERACIONAL),
      InpFractalAsas, InpJanelaBarras, InpMinTestes, InpBarrasGraca, InpStopGracaATRMult, InpTrailingATRMult, g_RiskPercent,
...
void OnDeinit(const int reason)
{
   ...
   ObjectDelete(0, PAINEL_RSR_NOME);
}
...
bool TemPosicaoAberta()
{
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      if(StringFind(comentario, "RSR ") == 0)
      {
...
//+------------------------------------------------------------------+
// Mesmo algoritmo do TRIVIUM_DESENHAR_SR.mq5 (fractal + fusao por
// tolerancia), mas calculado (nao desenhado) e limitado a barras JA
// CONFIRMAVEIS (defasagem de InpFractalAsas - nao usa futuro).
//+------------------------------------------------------------------+
struct NivelSR { double preco; int idxCriacao; };

int ColetarNiveis(MqlRates &rates[], int totalRates, int idxAtual, bool topos, NivelSR &niveis[])
{
   int total = 0;
   ArrayResize(niveis, 0);
   int inicio = MathMax(0, idxAtual - InpJanelaBarras);
   int fim = idxAtual - InpFractalAsas;
   if(fim <= inicio) return 0;

   for(int i = inicio; i < fim; i++)
   {
      bool confirma = true;
      for(int j = 1; j <= InpFractalAsas && confirma; j++)
      {
         if(i - j < 0 || i + j >= totalRates) { confirma = false; break; }
         if(topos)
         {
            if(rates[i].high < rates[i-j].high || rates[i].high < rates[i+j].high) confirma = false;
         }
         else
         {
            if(rates[i].low > rates[i-j].low || rates[i].low > rates[i+j].low) confirma = false;
         }
      }
      if(!confirma) continue;

      double novo = topos ? rates[i].high : rates[i].low;
      double tolerancia = novo * InpTolerancaPct / 100.0;
      bool fundiu = false;
      for(int k = 0; k < total; k++)
      {
         if(MathAbs(niveis[k].preco - novo) <= tolerancia) { niveis[k].preco = (niveis[k].preco + novo) / 2.0; fundiu = true; break; }
      }
      if(!fundiu && total < InpMaxNiveis)
      {
         ArrayResize(niveis, total + 1);
         niveis[total].preco = novo;
         niveis[total].idxCriacao = i;
         total++;
      }
   }
   return total;
...
int ContarTestes(MqlRates &rates[], double nivelPreco, int idxCriacao, int idxAtual)
{
   double tolerancia = nivelPreco * InpTolerancaPct / 100.0;
   int testes = 0;
   for(int i = idxCriacao + 1; i < idxAtual; i++)
      if(rates[i].low <= nivelPreco + tolerancia && rates[i].high >= nivelPreco - tolerancia) testes++;
   return testes;
}
...
void OnTick()
{
   ...
   int barrasNecessarias = InpJanelaBarras + InpFractalAsas + 5;
   MqlRates rates[];
   ArraySetAsSeries(rates, false); // ordem cronologica (indice 0 = mais antigo), igual ao backtest Python
   int copiados = CopyRates(_Symbol, PERIOD_OPERACIONAL, 0, barrasNecessarias, rates);
   if(copiados < barrasNecessarias) return;

   int idxSinal = copiados - 2;   // vela de sinal = indice 1 do grafico (ja fechada)
   int idxAnterior = copiados - 3;
   if(idxSinal < 0 || idxAnterior < 0) return;

   double range_sinal = rates[idxSinal].high - rates[idxSinal].low;
   long vol_soma = 0;
   for(int k = idxSinal - InpVolumePeriod; k < idxSinal; k++) vol_soma += rates[k].tick_volume;
   double vol_avg = (double)vol_soma / InpVolumePeriod;
   double vol_rel = (vol_avg > 0) ? (double)rates[idxSinal].tick_volume / vol_avg : 0;

   bool forca = (range_sinal >= InpForcaMin * atr_sinal) && (vol_rel >= InpForcaMin);
   if(!forca) return;

   NivelSR resistencias[], suportes[];
   int nRes = ColetarNiveis(rates, copiados, idxSinal, true, resistencias);
   int nSup = ColetarNiveis(rates, copiados, idxSinal, false, suportes);

   string side = "";
   double nivelRompido = 0;
   int testesNivel = 0;

   for(int i = 0; i < nRes && side == ""; i++)
   {
      if(rates[idxSinal].close > resistencias[i].preco && rates[idxAnterior].close <= resistencias[i].preco)
      {
         int testes = ContarTestes(rates, resistencias[i].preco, resistencias[i].idxCriacao, idxSinal);
         if(testes >= InpMinTestes) { side = "BUY"; nivelRompido = resistencias[i].preco; testesNivel = testes; }
      }
   }
   if(side == "")
   {
      for(int i = 0; i < nSup && side == ""; i++)
      {
         if(rates[idxSinal].close < suportes[i].preco && rates[idxAnterior].close >= suportes[i].preco)
         {
            int testes = ContarTestes(rates, suportes[i].preco, suportes[i].idxCriacao, idxSinal);
            if(testes >= InpMinTestes) { side = "SELL"; nivelRompido = suportes[i].preco; testesNivel = testes; }
         }
      }
   }
   if(side == "") return;
   ...
   string comentario = StringFormat("RSR %s %s", tf, side);
   if(StringLen(comentario) > 31) comentario = StringSubstr(comentario, 0, 31);

   string motivoDetalhado = StringFormat(
      "Rompimento S/R: nivel %s (%d testes previos, min %d) rompido por vela de forca (range=%.2fx ATR14, volume=%.2fx media20) - entra: %s",
      DoubleToString(nivelRompido, _Digits), testesNivel, InpMinTestes, range_sinal/atr_sinal, vol_rel, side);
...
double OnTester()
{
   ...
   if(h != INVALID_HANDLE)
   {
      ...
      FileWriteString(h, StringFormat("%s;%s;%s;%.1f;%d;%.2f;%.2f;%.2f;%.2f;%.2f;%d;%.2f;%.2f;%d\r\n",
         _Symbol, EnumToString(_Period), "ROMPIMENTOSR", g_RiskPercent, (int)AccountInfoInteger(ACCOUNT_LEVERAGE), deposito,
