//+------------------------------------------------------------------+
//|                            COLETA_HISTORICO_TRIVIUM_UNIVERSAL.mq5 |
//|                                          TRIVIUM369 / Cláudio     |
//|                                                                   |
//|   SCRIPT (roda UMA vez e encerra) — coleta de histórico direto    |
//|   do servidor da corretora para CSV.                              |
//|                                                                   |
//|   UNIVERSAL: serve para QUALQUER plataforma MT5.                  |
//|   Basta ajustar o input "SufixoAtivo":                            |
//|     - Admirals      ->  SufixoAtivo = "-T"   (EURUSD-T, ...)      |
//|     - MetaQuotes    ->  SufixoAtivo = ""     (EURUSD, ...)        |
//|   Ativos inexistentes na plataforma sao pulados automaticamente.  |
//+------------------------------------------------------------------+
#property script_show_inputs
#property strict

//=== PARÂMETROS EDITÁVEIS ==========================================
input string SufixoAtivo     = "-T";            // "-T"=Admirals | vazio ""=MetaQuotes
...
//=== ATIVOS (NOMES BASE, SEM SUFIXO) ===============================
string Bases[] =
{
   "EURUSD","GBPUSD","USDCHF","USDCAD","USDJPY",
   "USDSEK","USDCNH","NZDUSD","AUDUSD","AUDCAD",
   "XAUUSD","XAUGBP","XAUEUR","XAUCHF","XAUAUD"
};
...
void OnStart()
{
   int nAtivos = ArraySize(Bases);
   int nTFs    = ArraySize(TFs);
   int totalCombos = nAtivos * nTFs;

   int ok=0, parcial=0, falha=0, pulados=0;
   long totalBarras=0;

   PrintFormat("=== COLETA TRIVIUM369 (UNIVERSAL) ===");
   PrintFormat("Corretora: %s | Conta: %I64d | Sufixo: '%s'",
               AccountInfoString(ACCOUNT_COMPANY),
               AccountInfoInteger(ACCOUNT_LOGIN), SufixoAtivo);
   PrintFormat("%d combinacoes (%d ativos x %d TFs)", totalCombos, nAtivos, nTFs);

   for(int a=0; a<nAtivos; a++)
   {
      string base    = Bases[a];
      string simbolo = base + SufixoAtivo;     // ex: EURUSD-T  ou  EURUSD

      // garante o símbolo na Observação de Mercado
      if(!SymbolSelect(simbolo, true))
      {
         PrintFormat("[PULADO] Simbolo nao existe nesta plataforma: '%s' (%d TFs)",
                     simbolo, nTFs);
         pulados += nTFs;
         continue;
      }
      ...
      for(int t=0; t<nTFs; t++)
      {
         ...
         // pasta usa o nome BASE (sem sufixo); arquivo mantem o nome completo
         string pasta   = StringFormat("%s_%s", PastaBase, base);
         string arquivo = StringFormat("%s\\%s_%s.csv", pasta, TFNome(tf), simbolo);

         int h = FileOpen(arquivo, FILE_WRITE|FILE_TXT|FILE_ANSI);
         if(h == INVALID_HANDLE)
         {
            PrintFormat("[FALHA] Nao abriu %s (erro %d)", arquivo, GetLastError());
            ResetLastError();
            falha++;
            continue;
         }
   ...
   PrintFormat("=== RESUMO === OK=%d  PARCIAL=%d  FALHA=%d  PULADOS=%d  | TOTAL BARRAS=%I64d",
               ok, parcial, falha, pulados, totalBarras);
